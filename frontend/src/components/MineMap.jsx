  ...
  const html = `
    <div class="relative flex items-center justify-center cursor-pointer">
      ...
      <div class="relative flex flex-col items-center">
        <div class="w-10 h-10 rounded-full flex items-center justify-center font-mono font-bold text-xs text-white shadow-xl transition-all duration-300 ${isSelected ? 'ring-4 ring-cyan-400 scale-110' : ''}" 
...
export default function MineMap({
  ...
  return (
    <div className={`relative rounded-2xl overflow-hidden border border-slate-800 bg-slate-950 shadow-2xl flex flex-col ${className}`}>
      {/* Top Map Header / Controls */}
      <div className="bg-slate-950/90 backdrop-blur-md px-4 py-2.5 border-b border-slate-800 flex items-center justify-between z-20">
        <div className="flex items-center gap-2">
          <div className="p-1.5 bg-cyan-950/80 rounded-lg border border-cyan-800/60 text-cyan-400">
            <Compass className="w-4 h-4 animate-spin-slow" />
          </div>
          <div>
            <h2 className="text-sm font-bold text-white flex items-center gap-2">
              <span>Mine Pit Geospatial Radar</span>
              <span className="text-[11px] font-mono text-cyan-400 font-normal">Leaflet 1:5000</span>
            </h2>
          </div>
        </div>
      ...
      <div className="relative w-full h-full min-h-[380px] lg:min-h-[460px]">
                ...
                <Popup>
                  <div className="p-1 min-w-[210px] text-slate-200">
                    <div className="flex items-center justify-between gap-2 pb-2 border-b border-slate-700">
                      <span className="font-bold text-sm text-white">{zone.zone_name || zone.name || zId}</span>
                    ...
                    <div className="grid grid-cols-2 gap-2 my-2 text-xs font-mono">
                      <div>
                        ...
                        <strong className="text-white text-sm">{Number(zone.risk_score).toFixed(2)}</strong>
                      </div>
                      <div>
                        <span className="text-slate-400 block text-[10px]">Deformation:</span>
                        <strong className="text-white text-sm">{zone.deformation_mm} mm</strong>
                      </div>
                      <div>
                        <span className="text-slate-400 block text-[10px]">Rainfall:</span>
                        <span className="text-cyan-300">{zone.rainfall_mm} mm</span>
                      </div>
                      <div>
                        <span className="text-slate-400 block text-[10px]">Crack Severity:</span>
                    ...
                    <button
                      ...
                      className="w-full mt-1 py-1.5 bg-cyan-600 hover:bg-cyan-500 text-slate-950 font-bold rounded-lg text-xs transition-all text-center flex items-center justify-center gap-1"
                    >
        ...
        {/* Floating Map Legend */}
        <div className="absolute bottom-4 left-4 z-[400] bg-slate-950/85 backdrop-blur-md p-3 rounded-xl border border-slate-800 shadow-xl text-xs font-mono">
          <div className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-2">Mine Risk Map Legend</div>
          <div className="space-y-1.5">
            <div className="flex items-center gap-2">
              <span className="w-3 h-3 rounded-full bg-emerald-500 shadow-sm shadow-emerald-500/50"></span>
              <span className="text-slate-300">LOW (Risk &lt; 0.35) — Safe</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="w-3 h-3 rounded-full bg-amber-500 shadow-sm shadow-amber-500/50"></span>
              <span className="text-slate-300">MEDIUM (0.35 - 0.75) — Warning</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="w-3 h-3 rounded-full bg-red-500 shadow-sm shadow-red-500/50 animate-pulse"></span>
              <span className="text-slate-300">HIGH (&gt; 0.75) — Evacuation</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="w-3 h-3 rounded-full bg-slate-500"></span>
              <span className="text-slate-400">STALE — No Telemetry (&gt;15m)</span>
            </div>
          </div>
        </div>
      </div>
